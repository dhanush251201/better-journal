...
struct ContentView: View {
        ...
        NavigationStack {
            List {
                Section {
                    ...
                    .tint(BJDesign.Palette.warmOrange)
                }
            }
    ...
    private var carouselView: some View {
        ...
        return TabView {
            // Slide 1: Mood-aware quote (falls back to static message)
            ZStack(alignment: .bottomTrailing) {
                VStack(spacing: 10) {
                    Image(systemName: store.moodQuote != nil ? "sparkles" : "sun.max.fill")
                        .font(.title2)
                        .foregroundStyle(BJDesign.Palette.warmOrange)
                    Text(store.moodQuote ?? dailyMessage)
            ...
            if !store.moodSummaries.isEmpty {
                VStack(spacing: 8) {
                    ...
                    if let emotion = store.entries.first?.resolvedEmotion {
                        Text("Feeling \(emotion.displayName.lowercased())")
        ...
        HStack(spacing: 0) {
            ...
            VStack(alignment: .leading, spacing: 8) {
                ...
                VStack(alignment: .leading, spacing: 6) {
                    // Emotion tags — stacked vertically
                    ForEach(entry.resolvedEmotions, id: \.self) { emotion in
